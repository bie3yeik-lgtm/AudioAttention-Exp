...
ACTIONS_FAILURE_MODE = 'degraded'
DIAGNOSTICS = {
    'sources': {'manual_dates': 0, 'actions_runs': 0, 'planned_job_specs': 0, 'cron_occurrences': 0},
    'actions': {'successful_queries': 0, 'failed_queries': 0, 'errors': []},
    'deduplicated_jobs': 0,
    'excluded_reserved_jobs': 0,
    'excluded_current_run': 0,
}
...
def add_demand(schedule, day, workload, jobs, units, expected_cost_usd, source_id):
    ...
    if source_id.startswith('actions-run:'):
        DIAGNOSTICS['sources']['actions_runs'] += 1
    elif source_id.startswith('job-spec:'):
        DIAGNOSTICS['sources']['planned_job_specs'] += 1
    elif source_id.startswith('cron:'):
        DIAGNOSTICS['sources']['cron_occurrences'] += 1
    return True
    ...
    for pattern in cfg.get('globs', []):
        for path in glob.glob(pattern, recursive=True):
            ...
            if job_id and job_id in RESERVED_JOB_IDS:
                DIAGNOSTICS['excluded_reserved_jobs'] += 1
                continue
            if job_id and job_id in SEEN_JOB_IDS:
                DIAGNOSTICS['deduplicated_jobs'] += 1
                continue
            when = None
            ...
            if workload:
                added = add_demand(schedule, when.astimezone(tz).date().isoformat(), workload, jobs, units, cost, f'job-spec:{path}')
                if added and job_id:
...
def collect_runs(schedule, source_cfg, forecast_cfg, now, tz_name, repo, token, exclude_run_id, reserved_job_ids):
    ...
    if not repo or not token:
        DIAGNOSTICS['actions']['failed_queries'] += 1
        DIAGNOSTICS['actions']['errors'].append('missing_repo_or_token')
        if ACTIONS_FAILURE_MODE == 'fail_closed':
            raise RuntimeError('Actions forecast requires repository/token')
        return
    ...
    for status in cfg.get('statuses', ['queued', 'in_progress']):
        try:
            ...
            DIAGNOSTICS['actions']['failed_queries'] += 1
            DIAGNOSTICS['actions']['errors'].append(f'{status}:{type(exc).__name__}:{exc}')
            if ACTIONS_FAILURE_MODE == 'fail_closed':
                raise RuntimeError(f'Actions forecast query failed for {status}') from exc
            continue
        DIAGNOSTICS['actions']['successful_queries'] += 1
        for run in runs:
            run_id = str(run.get('id', ''))
            if exclude_run_id and run_id == str(exclude_run_id):
                DIAGNOSTICS['excluded_current_run'] += 1
                continue
            ...
            if workload:
                added = add_demand(schedule, max(when, now).astimezone(tz).date().isoformat(), workload, jobs, units, cost, f'actions-run:{run_id}')
                if decoded and added and job_id:
                    SEEN_JOB_IDS.add(job_id)


def main():
    global ACTIONS_FAILURE_MODE, RESERVED_JOB_IDS
    ...
    p.add_argument('--actions-failure-mode', choices=['degraded','fail_closed'], default=os.environ.get('GPU_BUDGET_FORECAST_ACTIONS_FAILURE_MODE') or None)
    args = p.parse_args()
    ...
    ACTIONS_FAILURE_MODE = args.actions_failure_mode or source_cfg.get('actions', {}).get('failure_mode', 'degraded')
    schedule = deepcopy(load_schedule(forecast_cfg['schedule_file']))
    schedule.setdefault('version', 1)
    schedule.setdefault('dates', {})
    DIAGNOSTICS['sources']['manual_dates'] = len(schedule.get('dates', {}))
    ...
    if args.bucket:
        try:
            ...
            RESERVED_JOB_IDS = set(reserved)
        except Exception:
            pass

    # Queue/running state wins over the same planned Job Spec.
    ...
    failed = DIAGNOSTICS['actions']['failed_queries']
    succeeded = DIAGNOSTICS['actions']['successful_queries']
    health = 'healthy' if failed == 0 else 'degraded'
    confidence = 'high' if failed == 0 else ('medium' if succeeded > 0 else 'low')
    schedule['generated'] = {
        'at': now.isoformat(),
        'horizon_end': horizon.isoformat(),
        'health': health,
        'confidence': confidence,
        'actions_failure_mode': ACTIONS_FAILURE_MODE,
        **DIAGNOSTICS,
    }
    ...
    print(json.dumps({'output': str(out), 'dates': len(schedule.get('dates', {})), 'health': health, 'confidence': confidence, 'diagnostics': DIAGNOSTICS}, ensure_ascii=False))
