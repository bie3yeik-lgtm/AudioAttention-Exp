...
if [ -n "${VAST_OFFER_ID:-}" ]; then
  ...
  # Explicitly require enough local disk, in addition to using --storage for pricing.
  query="${GPU_QUERY} disk_space>=${DISK_GB}"
  offers="$(
...
# --cancel-unavail prevents Vast from silently creating a stopped instance
# when the validated offer disappears between search and create.
result="$(
...
if not obj.get("success", True):
    raise SystemExit(f"Vast create failed: {obj}")
